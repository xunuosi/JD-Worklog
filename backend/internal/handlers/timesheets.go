...
func (h *TimesheetHandler) Create(c *gin.Context) {
	...
	if err != nil { c.JSON(http.StatusBadRequest, gin.H{"error":"bad date"}); return }
	ts := models.Timesheet{UserID: userID, ProjectID: req.ProjectID, Date: d, Hours: req.Hours, Content: req.Content}
	if err := h.DB.Create(&ts).Error; err != nil { c.JSON(http.StatusBadRequest, gin.H{"error":err.Error()}); return }
...
func (h *TimesheetHandler) Delete(c *gin.Context) {
	...
	if res.Error != nil { c.JSON(http.StatusInternalServerError, gin.H{"error":"db"}); return }
